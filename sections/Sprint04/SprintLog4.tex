...
\autoref{Tasks3}, which include the assessment of our various solutions from
...
T772    & Assess: Security problem when changing system date/time & ??\\\hline
...
the \textc{component-lib} library. This process consists of removing the old \textc{db-lib} and replacing
...
in the following examples. The first phase is to replace database requests with
the REST equivalent, and the second phase is to replace the helper and
controller classes which have been implemented by \textc{db-lib}. Additionally,
we discuss the changes made to the \textc{GirafPictogramItemView} class of the
...
In addition, we make a request to the database whenever we need to use data on a
...
start by removing all imports from this library. Then, since most of
\textc{db-lib's} functionality is contained in a number of helper classes, we
remove these, and look at what they were used for. An example of a helper class
could be the \textc{profileApplicationController} in the \textc{GirafFragment}
...
which can be seen in \autoref{ucaa}. In this method, we can simply access the
user's settings object, and check if the given application appears on the user's
list of applications. By doing this, we have actively replaced the necessity of
the \textc{db-lib} helpers.\nl
...
\textc{GetRequest} nested inside it, this can be seen on \autoref{logigReq} and
\autoref{userReq}.\nl
...
file for the login screen on \textbf{lines 3 \& 4}. We then create references to
the various UI elements, such as the textboxes on \textbf{lines 6-7}. On \textbf{line 11-22} we create a
...
pictures and along with the relevant part of the text to better reflect the new
design.

\subsubsection{T872: Document the Launcher}
We have tried to document the \textc{Launcher} in a brief fashion, see
\autoref{WikiLauncher}. On this page, we describe the noteworthy features, the
things a developer need to consider when implementing \textc{REST} and some of
the missing features we have not had time to create. We have also included a
short wiki page to describe the important parts of \textc{Component Lib},
see \autoref{WikiCompLib}. This page is mainly comprised of a description of the
classes we have made, and we advise that it should be recreated from scratch.
...
Throughout testing the launcher after the REST API was included, we see that when
we crash the \textc{GirafBugSplashActivity}, see \autoref{GBugSplash}, is no
...
new implementation was made but we have not been able to resolve it due to time
constraints. As such this task can only be considered partially resolved.

\subsubsection{T822: Assess the implementation of Grayscale}
In order to make use of REST, we changed the GrayScaleHelper class to include a
server request, which retrieves the user's settings object. This can be seen in
\autoref{NEWsetGrayScaleUser}. As seen on \textbf{line 6-10}, we make a simple
\textc{GetRequest} for the user, and then pass its settings object and the
current activity to the \textc{setGrayScaleForActivity} method.\nl
...
correctly, and as such, we use the \textc{root View} on \textbf{line 5} in 
\autoref{NEWsetGrayScale}. This functions the same as the old code, but does
not crash GIRAF applications.
...
\begin{itemize}
  ...
  \item When changing and setting settings for a citizen, it is not possible
  ...
  grayscale does not work outside \textc{SettingsActivity}.
  \item The apps the user can access is never sent to the server, because of a
  stack overflow in rest client lib, when that is fixed, fix the out commented
  code in the respective classes for that issue.
  \item A better icon should be created for missing apps on the device, an icon
  ...
  app installed when searching for an icon for a specific app.
\end{itemize}

The entirety of documented bugs and missing features can be found in
\autoref{fwork}, where we discuss what changes should be made to the Launcher
and component-lib library by next year's students.
...
\begin{itemize}
  \item T772: Assess: Security problem when changing system date/time
      \begin{itemize}
        ...
        logged in, using tokens/cookies which the rest library handles.
        \end{itemize}
  ...
  \item T889: Add an Institute to Guardian login \label{T889}
   \begin{itemize}
     ...
     institute-type user to be logged in as a guardian. 
...
During this sprint, there were several small disruptions related to \textc{REST}.
...
\textc{REST-Models} and \textc{REST-API-Client-Lib}, which meant that work was
...
managed to leave the \textc{Launcher} and \textc{Component-Lib} working, with
...
\textc{REST} worked and there was a lot of problem-solving between the groups.
...
as that was when further development on the \textc{REST} model stopped. This is
