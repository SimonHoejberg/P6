...
\begin{minipage}[H]{\linewidth}
\begin{lstlisting}[caption = ???, label = userReq]
new Response.Listener<Integer>() {
	@Override
...
users setting, see \autoref{NEWsetGrayScale}. As seen on \textbf{lines xx-xy} we
make a simple \textc{GetRequest<User>} pass the settings and activity to the
\textc{setGrayScaleForActivity} method.

\begin{minipage}[H]{\linewidth}
\begin{lstlisting}[caption = , label = NEWsetGrayScale] 
...
Old method didn't work on giraf as we could not access the current view

added a method to create a request for the users grayscalesetting

we changed it to get the root view which worked, and the code is now:

\subsubsection{T878: Fix bugs}
Fixing bugs is hard when you dont have an even remotely finished product.
