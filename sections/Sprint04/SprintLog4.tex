...
In the \textc{onCreate} method, see \autoref{onCreate}, we instantiate the
\textc{loginController}, set the current view as
\textc{R.layout.login\_activity}, which corresponds to the XML file for the
...
The \textc{onLoginButtonClick} method is used to disable the various elements
the user can interact with, as well starting the load animation. The
username and password are then passed to \textc{controller} via the
\textc{login} method.
...
The \textc{login} method is handled by first using our getting a \textc{request
queue}, which is discussed in \fix{}{add autoref}. We then begin with the login
