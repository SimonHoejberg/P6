...
the quality of the previosly written code is acceptable. The tasks for
this sprint are mostly superficial, but they do also include a number of fixes
to critical bugs.\nl
...
applications \textc{Launcher}, see \autoref{LauncherReview}, and
\textc{Pictogram Searcher}, see \autoref{PictoSearchReview}. While these
applications represent our current core responsibilities, not all of our tasks
are required to revolve around these apps.\nl

In this chapter we will describe each of the tasks we are assigned to as
well as their respective solutions. We will describe why a number of tasks are
removed and finally we will discuss the how the sprint retrospective.

\section{Tasks and Solutions}
Based on our area of responsibility and the backlog from the last semester's
students, we have been assigned an initial list of tasks, which can be seen in
...
\fix{Given that the rest of the system never expects the pictogram list to be
empty, this led to a problem where the system tried to access the first element of the list
which was null.}{Needs rework - Ying}\nl
...
an error-list for the \textc{Launcher project} can be seen in
...
another program to launch the Weekplanner provides an \textc{Intent}, this
