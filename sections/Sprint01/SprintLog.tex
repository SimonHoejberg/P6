...
\ttt{Activity}, the definition and assignment of \textc{loadAnimation} was contained
...
problem occours when running the \lapp\ on an emulator, because pinging the package
...
themselves check that a variable is not \ttt{null} such that the \ttt{Activity} exsist.
...
following UML-diagram, modelling the client-to-server interface:
...
20 each, such that the user may receive feedback in the form of a continously
updating list of pictograms.

\subsection{T683 - Weekplanner: Prevent starting without Launcher.} 
During inspection of the \wapp, group SW610 identified that
