...
catch NullPointerExceptions when we can simply check the variables instead.
...
example, if a user wants to find all pictograms related to \textc{cars}, the
program may queue multiple requests: one for all pictograms starting with
\textc{c}, then for all pictograms starting with \textc{ca}, and so on. This
