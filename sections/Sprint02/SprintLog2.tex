...
sometimes be logged out automatically before the degfined interval of 8
...
information about the login session is shared with the database in a possibly
faulty manner. This original approach can be seen in
...
the information to the database \textc{asynchronously}, where \textc{commit}
writes it \textc{synchronously}. Using \textc{commit} means that we are informed
if the writing failed, which allows us to attempt to share the login information
with the database multiple times, until it is correctly stored.\nl
...
problem to faulty communication with the database, which we have resolved by
using \textc{commit} instead of \textc{apply}.\nl
