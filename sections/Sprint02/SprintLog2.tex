...
Our initial area of responsibility is focused on the \textc{Launcher} and
\textc{Pictogram Searcher}, and for this sprint, we have chosen to focus more on
the \textc{Launcher} by improving the associated \textc{Login System}. We also
aim to refactor \textc{Pictogram Reader}. This choice is based on the fact,
...
semester's students only implemented the splash-screen and did not implement
the actual code which was supposed to output the stack trace.\nl

The \textc{giraf-component-lib} library contains the
\textc{GirafBugSplashActivity} class which is extended from \textc{GirafActivity}
...
The following is a task of a collaboration between SW609, SW610, and SW612 and is
...
\textc{apply} method to save the time of authentication
...
secondly, we could implement that it tries again until it is correctly
stored.\nl
...
problem to faulty communication with the shared preferences, which we have
...
Due to the complex nature of this task, and the cooperation with the other
...
application, which occurs if we press the \textc{Play} button while no
pictograms are chosen. This error occurs as the application attempts to access
...
pictogram in the list is not null, but that there are no safeguards to make sure
...
application. This entailed which color scheme to use, what each icon means and
...
different users navigate between \textc{activities} and the \textc{login
...
During the first sprint, group SW610 was testing the Weekplanner application and
noticed that some activities did not support vertical orientation of the screen.
Due to the inconsistency of only some activities supporting this, we have agreed
to implement vertical functionality in the pictosearch activity.\nl
...
form of a folder, containing an XML file specifying the GUI. This means, that
its resource folder \textc{res} must contain subfolders named \textc{layout},
...
system would automatically log in to the previous session. This issue can be split
...
\begin{itemize}
  ...
  \begin{itemize}
  	...
  	to find out if the current approach to switching from \textc{citizen} to
  	\textc{guardian} is correct. 
  	...
  	\textc{citizen} to \textc{guardian}. Therefore we have invalidated the task.
  \end{itemize}
