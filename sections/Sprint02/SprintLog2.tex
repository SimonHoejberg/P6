...
\subsection{T722 - Prevent login session from expiring}\label{T722}
...
covered how to to make each make each application recognizable as a GIRAF
applicaion. This entailed which color scheme to use, what each icon means and
how each application should be structured. Through T709, \autoref{sec:Colab2},
we found the requirements for the log-in screen, mainly to replace the current
QR-code with a password based system.

% The design is based on a design manual created during a previous semester. The
% purpose of the design manual is to create some design guidelines, such that every
...
\autoref{S2:NewReqs} we create a mockup for the new login system, see
\autoref{LoginScreen}.
...
citizens session will never expire.\fix{}{Maybe it should be moved to when it
is explicitly used?}
...
\begin{itemize}
  ...
  \end{itemize}
  \item T708 - Define GUI design guidelines
  \begin{itemize}
