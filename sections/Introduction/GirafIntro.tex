...
\figx[0.65]{ProjectStruct}{The project structure used this semester}
...
\textc{Weekplanner}, server and database.\nl

The \textc{Weekplanner} has been chosen, as it is the main application which is used
...
The user is able to login using a QR code, as seen in \autoref{Login}. In the
...
\figx[0.25]{Login}{Trying to log in using a QR code}
...
\figx[0.37]{MenuGuardian}{The guardians home menu screen} 
...
\wapp\ and \textc{Pictoreader}.\nl

\plib's visual components consists of a number of elements. At the top right is
...
\begin{enumerate}
  ...
  between \ttt{Activities}.
  \item The \lapp\ only loads user-data on startup. This can lead to outdated
  data being used.
  \item The \lapp\ uses the \ttt{DB-Lib} library for server communication, and needs
  to implement the new \ttt{REST} framework, which should be developed during the project.
\end{enumerate}
...
\begin{enumerate}   
  ...
  and commas.
  \item The library uses the \ttt{DB-Lib} library for server communication, and needs
  to implement the new \ttt{REST} framework, which should be developed during the project.
\end{enumerate}
