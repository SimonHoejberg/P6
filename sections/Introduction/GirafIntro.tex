...
testing purposes which skips the QR code. \fix{Do we need a citation to a
project from last semester? if yes point to Voice Game ch.15}{}

\figx[0.25]{Login}{The login screen using QR codes as of February 2017}
...
\figx[0.35]{MenuGuardian}{The guardians menu screen.}
...
similar button on the citizens menu, which means that a guardian cannot revert
from citizen to guardian without having to log out.
...
\figx[0.35]{OptionsGuardian}{The settings menu}
...
Button \textbf{3} is allows the guardian to control how many applications are
...
\autoref{OptionsGuardian2}. Button \textbf{7} as in the other menus provide the
