...
however, has appeared to be impossible to do because SW613 did not succed to
implement the REST client library in such a way that us application developers
should not work with the request and their responce and error listner as we do
...
more lines of code to retive a singlle user.\nl

Although the have made an handler which eliminates the need to instaniate the
request and add it to the queue. But we do still need to implement both
the onResponse and onError listner. So it remove 2 lines of code on anvage from
the request.\nl

If it was implemented as we dicussed at the start of the development using
